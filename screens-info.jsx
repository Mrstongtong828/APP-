...
function OrdersScreen(){
  ...
  return (
    <Page title="维修订单" onBack={()=>go('mine')}>
      <div style={{padding:'8px 14px',display:'flex',gap:20,background:'#fff',borderBottom:'1px solid #F1F5FB'}}>
        {['全部 12','待处理 1','维修中 1','已发货 10'].map((t,i)=>(
          <button key={i} style={{
...
function AddressScreen(){
  ...
  const [f, setF] = useState({
    name:'李医生', phone:'138 0013 8000',
    region:'广西壮族自治区 / 桂林市 / 象山区',
    detail:'中山中路 88 号 桂林口腔门诊 4 楼',
    unit:'桂林口腔门诊', def:true,
  });
  const upd = (k,v)=>setF(s=>({...s,[k]:v}));
  const Row = ({label, required, children, last})=>(
    <div style={{display:'flex',alignItems:'flex-start',gap:12,padding:'14px 14px',borderBottom:last?'none':'1px solid #F1F5FB',minHeight:48}}>
      <div style={{width:78,fontSize:13.5,color:'#324563',flexShrink:0,paddingTop:1}}>
        {required && <span style={{color:'#E5484D',marginRight:2}}>*</span>}{label}
      </div>
  ...
  return (
    <Page title="收货地址" onBack={()=>go('mine')}>
      <div style={{padding:14}}>
        <Card pad={0} style={{overflow:'hidden'}}>
          <Row required label="收货人">{Inp('name','请输入收货人姓名')}</Row>
          <Row required label="手机号码">{Inp('phone','请输入联系电话')}</Row>
          <Row required label="所在地区">
            {Inp('region','省 / 市 / 区')}
            <svg width="8" height="12" viewBox="0 0 8 12"><path d="M1 1l5 5-5 5" stroke="#C4D1E4" strokeWidth="1.6" fill="none" strokeLinecap="round"/></svg>
          </Row>
          <Row required label="详细地址">
            {Inp('detail','街道 / 门牌号 / 楼层')}
            <Glyph name="pin" size={18} color="#1E6FE0"/>
          </Row>
          <Row label="单位名称" last>{Inp('unit','诊所 / 医院 名称（选填）')}</Row>
        </Card>

        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',
          background:'#fff',borderRadius:12,padding:'14px 14px',marginTop:10}}>
          <div>
            <div style={{fontSize:14,color:'#0F1F3A',fontWeight:500}}>设为默认地址</div>
            <div style={{fontSize:11.5,color:'#94A3B8',marginTop:3}}>报修时将优先使用此地址</div>
          </div>
          <button onClick={()=>upd('def',!f.def)} style={{
            width:44,height:24,borderRadius:99,position:'relative',
            background:f.def?'#1E6FE0':'#D7E1EE',transition:'background .2s',
          }}>
            <span style={{position:'absolute',top:2,left:f.def?22:2,width:20,height:20,borderRadius:99,background:'#fff',
              boxShadow:'0 2px 4px rgba(0,0,0,.15)',transition:'left .2s'}}/>
          </button>
        </div>

        <div style={{padding:'20px 0 8px'}}><Btn full size="lg" onClick={()=>go('mine')}>保存地址</Btn></div>
        <button style={{
          width:'100%',padding:12,borderRadius:99,background:'transparent',
...
function FeedbackScreen(){
  ...
  const [contactKind, setContactKind] = useState('phone');
  const [text, setText] = useState('');
  const cfg = {
    phone: { label:'手机号码', ph:'请输入 11 位手机号码' },
    qq:    { label:'QQ 号码',  ph:'请输入 QQ 号' },
    email: { label:'邮箱地址', ph:'请输入常用邮箱' },
  }[contactKind];
  return (
    <Page title="投诉和建议" onBack={()=>go('mine')}>
      <div style={{padding:14}}>
        {/* Segmented 投诉 / 建议 */}
        <div style={{
          background:'#EAF0FA',borderRadius:99,padding:4,
          display:'grid',gridTemplateColumns:'1fr 1fr',gap:4,
        }}>
          {[{id:'建议',t:'建议'},{id:'投诉',t:'投诉'}].map(t=>{
            const on = t.id===type;
            return (
              <button key={t.id} onClick={()=>setType(t.id)} style={{
                padding:'10px 14px',borderRadius:99,fontSize:14,
                background:on?'#fff':'transparent',
                color:on?'#0A4FB8':'#6B7C97',fontWeight:on?700:500,
                boxShadow:on?'0 4px 12px -4px rgba(30,111,224,.25)':'none',
                transition:'all .15s',
              }}>{t.t}</button>
            );
          })}
        </div>
        <div style={{fontSize:11.5,color:'#94A3B8',marginTop:8,padding:'0 4px'}}>
          {type==='投诉'?'收到投诉后, 主管会在 24 小时内主动联系您':'欢迎提出您宝贵的建议, 采纳后可获赠小礼品'}
        </div>

        <Card pad={0} style={{marginTop:12,overflow:'hidden'}}>
          <div style={{padding:'14px 14px 6px',borderBottom:'1px solid #F1F5FB'}}>
            <div style={{fontSize:13,color:'#324563',fontWeight:500,marginBottom:8}}>
              <span style={{color:'#E5484D',marginRight:2}}>*</span>详细描述
            </div>
            <textarea value={text} onChange={e=>setText(e.target.value)}
              placeholder={type==='投诉'?'请描述问题发生的时间、经过以及您的诉求……':'请描述您的建议与期望, 我们会认真评估……'} rows={5}
              style={{width:'100%',border:'none',outline:'none',background:'transparent',
                fontSize:14,fontFamily:'inherit',resize:'none',color:'#0F1F3A',lineHeight:1.6}}/>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:4,paddingBottom:8}}>
              <span style={{fontSize:11,color:'#94A3B8'}}>可附 0/3 张图片</span>
              <span style={{fontSize:11,color:'#94A3B8'}}>{text.length}/500</span>
            </div>
          </div>

          <div style={{padding:'14px 14px 4px'}}>
            <div style={{fontSize:13,color:'#324563',fontWeight:500,marginBottom:10}}>
              <span style={{color:'#E5484D',marginRight:2}}>*</span>联系方式
            </div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:10}}>
              {[{id:'phone',t:'手机'},{id:'qq',t:'QQ'},{id:'email',t:'邮箱'}].map(c=>{
                const on = c.id===contactKind;
                return (
                  <button key={c.id} onClick={()=>setContactKind(c.id)} style={{
                    padding:'9px 10px',borderRadius:10,fontSize:13,
                    background:on?'#E8F1FE':'#fff',
                    border:`1.5px solid ${on?'#1E6FE0':'#E4ECF7'}`,
                    color:on?'#0A4FB8':'#324563',fontWeight:on?600:500,
                  }}>{c.t}</button>
                );
              })}
            </div>
          ...
          <div style={{padding:'0 14px 14px'}}>
            <div style={{background:'#F5F9FF',borderRadius:10,padding:'12px 14px',
              display:'flex',alignItems:'center',gap:10}}>
              <span style={{fontSize:12.5,color:'#6B7C97',flexShrink:0}}>{cfg.label}</span>
              <input placeholder={cfg.ph}
                style={{flex:1,minWidth:0,border:'none',outline:'none',background:'transparent',
                  fontSize:14,color:'#0F1F3A',fontFamily:'inherit',padding:0,textAlign:'right'}}/>
            </div>
          </div>
        </Card>

        <Card style={{marginTop:10}}>
          <Field label="关联工单" hint="选填 · 填写后便于我们快速定位问题">
            <Input value="" onChange={()=>{}} placeholder="如 DR-20260508-1147"/>
          </Field>
        </Card>

        <div style={{padding:'20px 0 8px'}}>
          <Btn full size="lg" onClick={()=>go('mine')}>提交{type}</Btn>
        </div>
        <div style={{fontSize:11,color:'#94A3B8',textAlign:'center',padding:'4px 0 12px'}}>
          提交后预计 1–3 个工作日内反馈结果
        </div>
      </div>
    </Page>
